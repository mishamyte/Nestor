DO $$

BEGIN 

IF NOT EXISTS (SELECT 1 FROM "public"."__MigrationHistory" WHERE "MigrationId" = '201806142132305_InitialCreate')
THEN

CREATE TABLE IF NOT EXISTS "public"."Nests"("Id" int4 NOT NULL DEFAULT 0,"PokemonId" int4 NOT NULL DEFAULT 0,"NestType" int4 NOT NULL DEFAULT 0,"Lat" float8 NOT NULL DEFAULT 0,"Lng" float8 NOT NULL DEFAULT 0,"IsRecommended" boolean NOT NULL DEFAULT FALSE,"LastMigration" int4 NOT NULL DEFAULT 0,CONSTRAINT "PK_public.Nests" PRIMARY KEY ("Id"))
;

CREATE INDEX IF NOT EXISTS "Nests_IX_PokemonId" ON "public"."Nests" ("PokemonId")
;

CREATE TABLE IF NOT EXISTS "public"."NestsUpdates"("Id" serial4 NOT NULL,"NestId" int4 NOT NULL DEFAULT 0,"PokemonId" int4 NOT NULL DEFAULT 0,"MigrationNumber" int4 NOT NULL DEFAULT 0,"Timestamp" timestamp NOT NULL DEFAULT '0001-01-01 00:00:00',CONSTRAINT "PK_public.NestsUpdates" PRIMARY KEY ("Id"))
;

CREATE INDEX IF NOT EXISTS "NestsUpdates_IX_NestId" ON "public"."NestsUpdates" ("NestId")
; 

CREATE INDEX IF NOT EXISTS "NestsUpdates_IX_PokemonId" ON "public"."NestsUpdates" ("PokemonId")
;

CREATE TABLE IF NOT EXISTS "public"."Pokemons"("Id" int4 NOT NULL DEFAULT 0,"Name" text,CONSTRAINT "PK_public.Pokemons" PRIMARY KEY ("Id"))
;

CREATE TABLE IF NOT EXISTS "public"."NestsInfo"("Id" int4 NOT NULL DEFAULT 0,"Name" text,"HashtagName" text,CONSTRAINT "PK_public.NestsInfo" PRIMARY KEY ("Id"))
;

ALTER TABLE "public"."Nests" DROP CONSTRAINT IF EXISTS "FK_public.Nests_public.Pokemons_PokemonId"
;

ALTER TABLE "public"."Nests" ADD CONSTRAINT "FK_public.Nests_public.Pokemons_PokemonId" FOREIGN KEY ("PokemonId") REFERENCES "public"."Pokemons" ("Id") ON DELETE CASCADE
;

ALTER TABLE "public"."NestsUpdates" DROP CONSTRAINT IF EXISTS "FK_public.NestsUpdates_public.Nests_NestId"
;

ALTER TABLE "public"."NestsUpdates" ADD CONSTRAINT "FK_public.NestsUpdates_public.Nests_NestId" FOREIGN KEY ("NestId") REFERENCES "public"."Nests" ("Id") ON DELETE CASCADE
;

ALTER TABLE "public"."NestsUpdates" DROP CONSTRAINT IF EXISTS "FK_public.NestsUpdates_public.Pokemons_PokemonId"
;

ALTER TABLE "public"."NestsUpdates" ADD CONSTRAINT "FK_public.NestsUpdates_public.Pokemons_PokemonId" FOREIGN KEY ("PokemonId") REFERENCES "public"."Pokemons" ("Id") ON DELETE CASCADE
;

CREATE TABLE IF NOT EXISTS "public"."__MigrationHistory"("MigrationId" varchar(150) NOT NULL DEFAULT ',"ContextKey" varchar(300) NOT NULL DEFAULT ',"Model" bytea NOT NULL DEFAULT ',"ProductVersion" varchar(32) NOT NULL DEFAULT ',CONSTRAINT "PK_public.__MigrationHistory" PRIMARY KEY ("MigrationId","ContextKey"))
;

INSERT INTO "public"."__MigrationHistory"("MigrationId","ContextKey","Model","ProductVersion") VALUES (E'201806142132305_InitialCreate',E'Nestor.Domain.Migrations.Configuration',decode('', 'base64'),E'6.2.0-61023')
;

END IF;

END $$;